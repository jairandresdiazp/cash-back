...
const calculateCashBack = async (orderForm) => {
    const currencySymbol = (r = 0, orderForm.storePreferencesData.currencySymbol);
    ...
    for (let indexItem = 0; indexItem < items.length; indexItem++) {
        ...
        await $.ajax({
            ...
            for (let indexSkuSpecifications = 0; indexSkuSpecifications < skuSpecifications.length; indexSkuSpecifications++) {
                const skuSpecification = skuSpecifications[indexSkuSpecifications];
                if (skuSpecification.FieldName == 'CashBack') {
                    try {
                        cashBackSpecification = skuSpecification.FieldValues[0]
                    } catch (error) {

                    }
                }
            }
            const cashBack = (item.price * item.quantity) * (cashBackSpecification / 100);
    ...
    try {
        const HTMLCashBack = `\n <tr class="cashBack-row">\n <td class="info">Cash Back</td>\n <td class="space"></td>\n<td class="monetary">${currencySymbol} ${new Intl.NumberFormat("es-CO").format(cashBackValue)}</td>\n <td class="empty"></td>\n</tr>`
